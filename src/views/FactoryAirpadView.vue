<script setup>
// 帐篷工厂介绍页（/factoryAir）。工厂介绍页第一张卡「查看」的落地页。
// 设计稿为 1920 宽英文版，除「产品轮播」和「视频」外全部为整幅切图
// （src/images/factoryAir/pc，编号 1-6 按设计稿自上而下的顺序）。
...
const OSS_FA = 'https://wolfwalkershop.oss-cn-beijing.aliyuncs.com/images/factoryAir/pc';
const heroPhoto = `${OSS_FA}/1.png`;
const heroText = `${OSS_FA}/2.png`;
const flowImage = `${OSS_FA}/3.jpg`;
const gridImage = `${OSS_FA}/4.jpg`;
const qcLeft = `${OSS_FA}/5.png`;
const qcRight = `${OSS_FA}/6.png`;

const AIRPAD_VIDEO =
  'https://wolfwalkershop.oss-cn-beijing.aliyuncs.com/images/detail/video/airpad.mp4';

const copy = {
  zh: {
    seriesTitle: '充气卧垫产品系列',
    seriesNote: 'WOLFWALKER 充气卧垫全系列',
    videoAlt: '充气卧垫工厂视频',
  },
  en: {
    seriesTitle: 'AIRPAD PRODUCT SERIES',
    seriesNote: 'WOLFWALKER full airpad range',
    videoAlt: 'Airpad factory video',
  },
};
...
// 轮播数据：充气卧垫分类下的全部产品，卡片样式与产品分类页共用 ProductCard。
const airpadProducts = products.filter((item) => item.category === 'airpad');
  ...
  <div class="factory-tent bg-[#f4f6f9]">
    ...
    <section class="py-12 lg:py-16">
      ...
      <div class="relative px-6 lg:px-16">
        ...
        >
          <ProductCard
            v-for="(product, index) in airpadProducts"
            :key="product.sku"
            v-reveal
            :product="product"
            :priority="index < 5"
